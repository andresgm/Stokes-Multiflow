clear;clc;
% clear all; %sigmav excesarea minxvert velcentx3 sedrate;
sbar = systembar();
...
%     saveas(2,[nameydir 'xvert'],'pdf')
    ...
    figure(2); plot(tiempov,excesarea);
    xlabel('Time (t)'); ylabel('Excess area (\alpha)');
    saveas(2,[nameydir 'excessarea'],'fig')
    saveas(2,[nameydir 'excessarea'],'pdf')

    figure(3); plot(tiempov,minxvert);
    xlabel('Dimensionless time');
    ylabel('lowest surface vesicle position');
    saveas(3,[nameydir 'minxvert'],'fig')
    saveas(3,[nameydir 'minxvert'],'pdf')
    
    figure(4); plot(tiempov,sigmav);
    xlabel('Time (t)'); ylabel('\Sigma');
    saveas(4,[nameydir 'sigma'],'fig')
    saveas(4,[nameydir 'sigma'],'pdf')
...
%     disp(['sigma = ' num2str(sigmav(end))])
    ...
    R0 = parms.R0;
    kbT = parms.kbT;
    kbar = parms.bending.kbar;
    disp(['R0 = ' num2str(parms.R0)])
    disp(['g0 = ' num2str(parms.g0)])
    disp(['minxvert = ' num2str(minxvert(end))])
    disp(['excesarea = ' num2str(excesarea(end))])
    disp(['Sigma = ' num2str(sigmav(end))])
    disp(['sigma = ' num2str(sigmav(end)*kbar*kbT/R0)])
    % posicion mas baja de la gota
%     disp(['minxvert = ' num2str(minxvert(end))])
%     % velocidad (rapidez) del centroide de la gota
